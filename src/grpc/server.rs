...
use crate::grpc::proto::{
    ...
    EmbedRequest, EmbedResponse, HealthRequest, HealthResponse,
...
impl EmbeddingService for EmbeddingServiceImpl {
    ...
    async fn embed(&self, _request: Request<EmbedRequest>) -> Result<Response<EmbedResponse>, Status> {
        Err(Status::unimplemented("Embed is handled by the Python embedding service"))
    }
    ...
    async fn health(&self, _request: Request<HealthRequest>) -> Result<Response<HealthResponse>, Status> {
        Ok(Response::new(HealthResponse { ready: false, model_name: String::new(), model_load_time_ms: 0 }))
    }
    ...
    async fn model_info(&self, _request: Request<ModelInfoRequest>) -> Result<Response<ModelInfoResponse>, Status> {
        Err(Status::unimplemented("ModelInfo is handled by the Python embedding service"))
    }
    ...
    async fn embed_stream(
        ...
        Err(Status::unimplemented("EmbedStream is handled by the Python embedding service"))
    }
}
